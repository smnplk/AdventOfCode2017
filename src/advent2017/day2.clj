...
(def numbers [[1136	1129	184	452	788	1215	355	1109	224	1358	1278	176	1302	186	128	1148]
...
(defn calc-difference [[min max]]
  (- max min))

(def process-numbers (comp
                      (map find-min-max)
                      (map calc-difference)))

(transduce process-numbers + numbers)


;; PART 2

(defn find-evenly-divisable-pair [v]
  (for [num v denom v :when (and (not= num denom) (zero? (rem num denom)))]
    [num denom]))

(map find-evenly-divisable-pair numbers)

(def process-numbers2 (comp
                       (map find-evenly-divisable-pair)
                       (map first)
                       (map (partial apply /))))

(transduce process-numbers2 + numbers)
